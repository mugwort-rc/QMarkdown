  ...
  XStr(const char* const toTranscode, XERCES_CPP_NAMESPACE_QUALIFIER MemoryManager *mm = 0)
        ...
        mm_(mm)
    {
      if(toTranscode == 0) {
        ...
        std::unique_ptr<unsigned char> charSizes(new unsigned char[needed]);
...
QString convert(const XMLCh* src)
{
    ...
    std::string result;
    std::unique_ptr<unsigned char> buffer(new unsigned char[length+1]);
    while ( size > eated ) {
    ...
    return QString::fromUtf8(result.c_str(), result.size());
}
...
public:
    static std::shared_ptr<ElementTreeImpl> generate()
    {
        ...
        return std::shared_ptr<ElementTreeImpl>(new ElementTreeImpl(impl->createDocument(nullptr, nullptr, nullptr)));
    }

    static std::shared_ptr<ElementTreeImpl> generate(const QString &root)
    {
        const XMLCh LS[] = {xercesc::chLatin_L, xercesc::chLatin_S, xercesc::chNull};
        xercesc::DOMImplementation *impl = xercesc::DOMImplementationRegistry::getDOMImplementation(LS);
        return std::shared_ptr<ElementTreeImpl>(new ElementTreeImpl(impl->createDocument(nullptr, X(root.toUtf8().constData()), nullptr)));
    }
...
{}
Element::Element() :
    Element(Element::InvalidElement)
{}

Element::Element(const ElementTree &doc) :
    _impl(new ElementImpl(doc._impl->ptr()->getDocumentElement()))
{}
Element::Element(const ElementTree &doc, const QString &name) :
    _impl(new ElementImpl(doc._impl->ptr()->createElement(X(name.toUtf8().constData()))))
{}
Element::Element(const Element &parent, const QString &name) :
    _impl(new ElementImpl(parent._impl->ptr()->getOwnerDocument()->createElement(X(name.toUtf8().constData()))))
{}
...
{
    return ElementTree(std::shared_ptr<ElementTreeImpl>(new ElementTreeImpl(this->_impl->ptr()->getOwnerDocument())));
}
...
Element::List Element::getElementsByTagName(const QString &name) const
{
    List result;
    xercesc::DOMNodeList *list = this->_impl->ptr()->getElementsByTagName(X(name.toUtf8().constData()));
...
QString Element::getTagName(void) const
{
    return convert(this->_impl->ptr()->getTagName());
}

void Element::setAttribute(const QString &key, const QString &val)
{
    this->_impl->ptr()->setAttribute(X(key.toUtf8().constData()), X(val.toUtf8().constData()));
}
...
Element::Attributes Element::getAttributes(void) const
{
    ...
    for ( XMLSize_t i = 0; i < attrs->getLength(); ++i ) {
        ...
        result[convert(attr->getNodeName())] = convert(attr->getNodeValue());
    }
    return result;
}

QString Element::getNamespaceURI(void) const
{
    return convert(this->_impl->ptr()->getNamespaceURI());
}

QString Element::getTextContent(void) const
{
    return convert(this->_impl->ptr()->getTextContent());
}
...
{
    if ( this->hasText() ) {
        this->_impl->ptr()->getFirstChild()->setNodeValue(X(text.toUtf8().constData()));
    } else {
        xercesc::DOMElement *elem = this->_impl->ptr();
        xercesc::DOMDocument *doc = elem->getOwnerDocument();
        xercesc::DOMText *node = doc->createTextNode(X(text.toUtf8().constData()));
...
void Element::setTail(const QString &tail)
{
    if ( this->hasTail() ) {
        this->_impl->ptr()->getNextSibling()->setNodeValue(X(tail.toUtf8().constData()));
    } else {
        xercesc::DOMElement *elem = this->_impl->ptr();
        xercesc::DOMDocument *doc = elem->getOwnerDocument();
        xercesc::DOMText *node = doc->createTextNode(X(tail.toUtf8().constData()));
...
QString Element::text(void) const
{
    if ( this->hasText() ) {
        return convert(this->_impl->ptr()->getFirstChild()->getNodeValue());
    }
    return QString();
}
QString Element::tail(void) const
{
    if ( this->hasTail() ) {
        return convert(this->_impl->ptr()->getNextSibling()->getNodeValue());
    }
    return QString();
}
...
ElementTree::ElementTree() :
    ElementTree(ElementTreeImpl::generate())
{}

ElementTree::ElementTree(const QString &root) :
...
void ElementTree::_setroot(const Element &elem)
{
    xercesc::DOMDocument *doc = this->_impl->ptr();
    auto current = doc->getDocumentElement();
    if ( current ) {
        doc->removeChild(current);
    }
    xercesc::DOMElement *node = elem._impl->ptr();
    if ( ! doc->isSameNode(node->getOwnerDocument()) ) {
        node = dynamic_cast<xercesc::DOMElement *>(doc->importNode(elem._impl->ptr(), true));
    }
...
{
    auto impl = std::make_shared<ElementImpl>(this->_impl->ptr()->getDocumentElement());
    return Element(impl);
}

QString ElementTree::toString() const
{
    const XMLCh LS[] = {xercesc::chLatin_L, xercesc::chLatin_S, xercesc::chNull};
    xercesc::DOMImplementation *impl = xercesc::DOMImplementationRegistry::getDOMImplementation(LS);
    ...
    if (serializer->getDomConfig()->canSetParameter(xercesc::XMLUni::fgDOMXMLDeclaration, false)) {
        serializer->getDomConfig()->setParameter(xercesc::XMLUni::fgDOMXMLDeclaration, false);
    }

    serializer->write(this->_impl->ptr(), output);
    serializer->release();

    QString result = QString::fromUtf8(reinterpret_cast<const char *>(target.getRawBuffer()));
    output->release();
